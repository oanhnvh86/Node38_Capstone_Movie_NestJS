...
async getMovieListPerDayPerPage (fromDate: string,toDate:string,page:string, size:string ): Promise<any>  {
    // let {fromDate, toDate} = req.query;
    // let {page, size} = req.params;
    let num_page = Number(page);
    let num_size = Number(size);
...
async createMovie(body: movieDTO): Promise<any>  {
    try{
        let {movie_name,trailer,image, description,premiere_date, rate, is_hot, is_showing_date,is_upcoming_date } = body;
